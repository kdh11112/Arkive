...
public class EgovFormBasedUUID implements Serializable {
	...
	private static Logger logger = LoggerFactory.getLogger(EgovFormBasedUUID.class);
    ...
    public static EgovFormBasedUUID nameUUIDFromBytes(byte[] name) {
        ...
        try {
			...
			logger.error("nameUUIDFromBytes InternalError", e);
		}catch (RuntimeException e) {
			logger.error("nameUUIDFromBytes RuntimeException", e);
		}
    ...
    public static EgovFormBasedUUID fromString(String name) {
        ...
        try{
	        ...
	        mostSigBits = Long.decode(components[0]).longValue();
	        mostSigBits <<= 16;
	        mostSigBits |= Long.decode(components[1]).longValue();
	        mostSigBits <<= 16;
	        mostSigBits |= Long.decode(components[2]).longValue();
	
	        leastSigBits = Long.decode(components[3]).longValue();
	        leastSigBits <<= 48;
	        leastSigBits |= Long.decode(components[4]).longValue();
	        
        } catch (IllegalArgumentException e) {
			logger.error("fromString IllegalArgumentException", e);
		}
    ...
    public long timestamp() {
    	...
    	try {
			...
			logger.error("timestamp UnsupportedOperationException", e);
		}
        return result;
    }
    ...
    public int clockSequence() {
    	try {
			...
			logger.error("clockSequence UnsupportedOperationException", e);
		}
    ...
    public long node() {
    	try {
			...
			logger.error("node UnsupportedOperationException", e);
		}
        return node;
    }
